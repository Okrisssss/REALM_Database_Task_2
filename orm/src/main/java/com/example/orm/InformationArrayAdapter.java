...
class InformationArrayAdapter extends ArrayAdapter<String> {
    ...
    private List list;
    private Dao<InfoModel,Integer>information_Dao;
    public InformationArrayAdapter(Context context, int resource, List objects,
                                   Dao<InfoModel, Integer> information_Dao) {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        if (list.get(position).getClass().isInstance(new InfoModel())){
            final InfoModel information_model = (InfoModel)list.get(position);
